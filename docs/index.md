# 欢迎来到工试2523の笔记本

> 📚 浙江大学工科试验班 2523 课程笔记与资料整理

这里记录并分享大学课程的学习笔记、考试资料和学习心得。

---

## 📚 课程列表

<div class="grid cards" markdown>

-   :material-calculator: **微积分（甲）II**

    函数、极限、导数、积分等基础知识

    [:octicons-arrow-right-24: 进入课程](calculus/index.md)

-   :material-matrix: **线性代数**

    矩阵、向量空间、线性变换

    [:octicons-arrow-right-24: 进入课程](linear-algebra/index.md)

-   :material-code-braces: **C程序设计**

    C语言编程基础与实践

    [:octicons-arrow-right-24: 进入课程](c-programming/index.md)

-   :material-drawing: **工程图学**

    工程制图原理与技术标准

    [:octicons-arrow-right-24: 进入课程](engineering-graphics/index.md)

-   :material-translate: **大学英语**

    英语综合能力提升

    [:octicons-arrow-right-24: 进入课程](college-english/index.md)

-   :material-chart-bell-curve: **常微分方程**

    一阶与高阶微分方程、线性方程与应用

    [:octicons-arrow-right-24: 进入课程](ode/index.md)

-   :material-atom: **大学物理（甲）I**

    力学、振动波动、热学与电磁学基础

    [:octicons-arrow-right-24: 进入课程](physics/index.md)

-   :material-ruler-square: **机械制图**

    机械图样阅读、习题答案与图纸资料

    [:octicons-arrow-right-24: 进入课程](mechanical-drawing/index.md)

-   :material-robot-outline: **人工智能基础（A）**

    人工智能入门讲义与课程课件整理

    [:octicons-arrow-right-24: 进入课程](ai-fundamentals/index.md)

-   :material-newspaper-variant-outline: **形势与政策I**

    课程笔记、时事资料与考前复习内容

    [:octicons-arrow-right-24: 进入课程](politics/index.md)

</div>

---

## 🎯 快速开始

1. **浏览课程** - 从上方课程列表选择感兴趣的课程
2. **下载资料** - 每门课程页面提供 PDF、代码等资料下载
3. **搜索内容** - 使用页面顶部搜索框快速查找内容
4. **切换主题** - 右上角可切换浅色/暗色模式

---

## 📝 关于本站

- **构建工具**: [Material for MkDocs](https://squidfunk.github.io/mkdocs-material/)
- **模板来源**: [TonyCrane/note](https://github.com/TonyCrane/note)
- **源码仓库**: [GitHub](https://github.com/Ayanami-WU/Engineer-2523-Notes-Gala)

## 📮 联系方式

- **GitHub**: [@Ayanami-WU](https://github.com/Ayanami-WU)
- **邮箱**: wumingxuan@zju.edu.cn
